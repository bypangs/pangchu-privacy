...
- 내부 에이전트의 이름, 역할, 조직도, 보고 체계, 검토 절차는 채팅 내부 협업 정보로만 취급하며 홈페이지·공개 HTML·메타·구조화 데이터·공지에 노출하지 않는다.
...
- 대쉬 게임 한국어 앱 이름은 `말랑냥이대쉬`로 통일한다.
- 대쉬 게임 영어/미국 앱 이름은 `Jelly Cat Dash`로 통일한다.
- 대쉬 게임 일본어 앱 이름은 `ぷにねこダッシュ`로 통일한다.
- 대쉬 게임 번체/홍콩 앱 표시명은 `啫喱貓衝刺`로 통일한다.
- 대쉬 게임 브라질 포르투갈어 앱 표시명은 `Gatinho Corre-Corre`로 통일한다.
...
- 대쉬 게임명을 임의로 `Malrangnyangi Dash`, `Jelly Cat Run`, `말랑냥이런`, `ネコニャンラン` 등으로 변경하지 않는다.
- 대쉬 게임 공지는 홈페이지의 공지사항/현재 공지 영역에서만 다루고, 사용자가 명시적으로 요청하기 전까지 Featured Game이나 메인 H1을 대쉬 게임 기준으로 바꾸지 않는다.
